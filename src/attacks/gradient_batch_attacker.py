...
class GradientBatchAttacker():
	...
	def __init__(self, x_lim, stopping_condition="n_steps", max_n_steps=10, early_stopping_delta=1e-2, early_stopping_patience=5, lr=1e-3, projection_stop_threshold=1e-3, projection_lr=1e-3, projection_time_limit=60, n_samples=20):
	...
	def project(self, phi_fn, x):
		...
		# while torch.max(loss) > self.projection_stop_threshold:
		while True:
	...
	def step(self, objective_fn, phi_fn, x):
		...
		normal_to_manifold = grad([torch.sum(phi_val)], x_batch)[0]
		x_batch.requires_grad = False
