...
class Trainer():
	...
	def train(self, objective_fn, reg_fn, phi_fn, xdot_fn):
		...
		while True:
			# Inner min
			# x = self.attacker.opt(objective_fn, phi_fn) # TODO: remove after debugging
			...
			if _iter % self.args.n_checkpoint_step == 0:
				...
				# save data too
				save_dict = {"test_losses": test_losses, "test_attack_losses": test_attack_losses, "test_reg_losses": test_reg_losses, "timings": timings, "train_attacks": train_attacks, "train_loss_debug": train_loss_debug, "train_attack_X_init": train_attack_X_init, "train_attack_X_final": train_attack_X_final, "a_grad":a_grad, "ci_grad":ci_grad}
