...
def simulate_quadrotor(xdot_fn, u_fn, timesteps=1000):
	...
	eps = np.random.normal(scale=1.0, size=12)  # let x0 be approximately in the 0-centered norm ball
	target = np.zeros(12)  # Stabilize to origin
	xcurr = target + eps
	x_all.append(xcurr)
	...
	for t in range(timesteps):
		...
		dist = np.linalg.norm(target - xcurr)
...
if __name__ == "__main__":
	...
	# print(xdot_fn(np.zeros(12), np.array([m*g, 0, 0, 0])))
	...
	# plt.savefig("plots/dist_q_%f_r_%f.png" % (q, r))
	plt.show()
	plt.clf()
	...
	# fig.savefig("plots/u_q_%f_r_%f.png" % (q, r))
	plt.clf()
	...
	fig = plt.figure()
	ax = fig.add_subplot(111, projection='3d')
	x_all = np.array(x_all)  # 1001 x 12
	# IPython.embed()
