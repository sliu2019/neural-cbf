...
class Phi(nn.Module):
	...
	def __init__(self, h_fn, xdot_fn, r, x_dim, u_dim, args):
		...
		self.ci = nn.Parameter(5*torch.rand(r-1, 1)) # int from 0-5 (if ci in small range, ki will be much larger) # TODO
...
def main(args):
	...
	test_attacker = GradientBatchAttacker(x_lim, max_n_steps=1)
	...
	# load_model(phi_fn, "./checkpoint/cartpole_default/checkpoint_2.pth")
	# print(list(phi_fn.parameters()))
	# IPython.embed()
