...
def simulate_rollout(x0, N_dt=100):
	...
	for t in range(N_dt):
		u, debug_dict = compute_u_ours(t, x) # Define this
		x_dot = x_dot_open_loop(x, u)
		x = x + dt*x_dot
...
if __name__ == "__main__":
	...
	for i in range(N_rollout):
		info_dict = simulate_rollout(x0s[i], N_dt)
	...
	plt.savefig("./rollouts/%s/%s_trajectories.png" % (log_folder, log_fname), bbox_inches='tight')
	plt.clf()
	plt.close()
