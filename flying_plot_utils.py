...
from main import SaturationRisk, RegularizationLoss
...
def load_phi_and_params(exp_name=None, checkpoint_number=None):
	...
	# Create phi
	from problems.quad_pend import RhoMax, Rho, XDot, ULimitSetVertices
	# if args.rho == "sum":
	h_fn = Rho(param_dict)
	# elif args.rho == "max":
...
def plot_invariant_set_slices(phi_star_fn, param_dict, samples=None, rollouts=None, which_params=None, constants_for_other_params=None, fnm=None, fldr_path=None, checkpoint=None):
	...
	for i in range(n_row):
		for j in range(n_per_row):
			...
			phi_vals = []
			for k in range(math.ceil(all_size/batch_size)):
				print("%i/%i" % (k, math.ceil(all_size/batch_size)))
				batch_input = input[k*batch_size: min(all_size, (k+1)*batch_size)]
				batch_input = batch_input.astype("float32")
				batch_input_torch = torch.from_numpy(batch_input)
			...
			# print(S_vals)
			print(np.mean(S_vals), np.std(S_vals), np.max(S_vals))
