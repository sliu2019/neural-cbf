...
class FlyingInvertedPendulumEnv():
	...
	def _f(self, x):
		...
		f = np.array([x[self.i["dgamma"]], x[self.i["dbeta"]], x[self.i["dalpha"]], 0, 0, 0, dphi, dtheta, ddphi, ddtheta])
		return f
	...
	def _g(self, x):
		...
		ddgamma = (1.0/self.J_x)*R
		ddbeta = (1.0/self.J_y)*R
		ddalpha = (1.0/self.J_z)*R
		...
		# Excluding translational motion
		g = np.zeros((self.x_dim, self.u_dim))
		g[3:6, 1:] = dd_drone_angles
		g[-2, 0] = ddphi
		g[-1, 0] = ddtheta
		# g = np.array([0, 0, 0, ddgamma, ddbeta, ddalpha, 0, 0, ddphi, ddtheta])
...
if __name__ == "__main__":
	...
	other_x_dot = f + g@u
	print(other_x_dot)
	print(np.isclose(x_dot, other_x_dot))
	# print(x, u)
	# print(x_dot)"""